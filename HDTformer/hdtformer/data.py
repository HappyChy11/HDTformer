...
def _construct_short_features(
    frame: pd.DataFrame,
    target_column: str,
    timestamp_column: str,
    feature_config: dict | None,
) -> tuple[pd.DataFrame, list[str]]:
    """Construct the paper's causal short-term features before splitting.

    Feature order is deterministic: target RV, shifted HEMA features in the
    configured span order, and the one-step RV difference. Every engineered
    value at time t uses observations no later than t-1, except the original
    RV value itself.
    """
    keep = [target_column]
    if timestamp_column in frame.columns:
        keep.insert(0, timestamp_column)
    prepared = frame[keep].copy()
    ...
    if not feature_config or not bool(feature_config.get("enabled", False)):
        return prepared.dropna(subset=[target_column]).reset_index(drop=True), [target_column]

    spans = [int(span) for span in feature_config.get("hema_spans", [])]
    if any(span < 1 for span in spans) or len(set(spans)) != len(spans):
        raise ValueError("hema_spans must contain unique positive integers")

    feature_columns = [target_column]
    for span in spans:
        column = f"hema_{span}"
        prepared[column] = (
            prepared[target_column].ewm(span=span, adjust=False).mean().shift(1)
        )
        feature_columns.append(column)

    if bool(feature_config.get("include_difference", True)):
        difference_column = str(
            feature_config.get("difference_name", "rv_difference")
        )
        prepared[difference_column] = prepared[target_column].diff()
        feature_columns.append(difference_column)

    warmup = int(feature_config.get("warmup", max(spans, default=0)))
    if warmup < 0:
        raise ValueError("feature warmup must be non-negative")
    prepared = prepared.iloc[warmup:].dropna(subset=feature_columns).reset_index(drop=True)
...
def load_and_split_data(
    ...
    frame, feature_columns = _construct_short_features(
        frame, target_column, timestamp_column, feature_config
    )
...
def load_external_split(
    ...
    raw_inputs = frame[feature_columns].to_numpy(dtype=np.float32)
    raw_targets = frame[[target_column]].to_numpy(dtype=np.float32)
    timestamps = (
    ...
    return SplitData(
        name=name,
        values=input_scaler.transform(raw_inputs).astype(np.float32),
        targets=target_scaler.transform(raw_targets).astype(np.float32),
...
def load_external_time_range(
    ...
    selected_columns = [timestamp_column, *feature_columns]
    selected = frame.loc[mask, selected_columns].copy()
    ...
    raw_inputs = selected[feature_columns].to_numpy(dtype=np.float32)
    raw_targets = selected[[target_column]].to_numpy(dtype=np.float32)
    return SplitData(
        name=name,
        values=input_scaler.transform(raw_inputs).astype(np.float32),
        targets=target_scaler.transform(raw_targets).astype(np.float32),
